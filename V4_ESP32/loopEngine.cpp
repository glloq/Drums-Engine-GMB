...
void LoopEngine::update() {
  ...
  // Advance ticks based on elapsed time
  while (elapsed >= _tickIntervalUs) {
    ...
    // Trigger events at current tick
...
int LoopEngine::createLoop(const char* name, uint16_t bpm, uint8_t bars,
  ...
  loop.ppq = 96; // Standard MIDI resolution
  loop.eventCount = 0;
  loop.active = true;
...
void LoopEngine::quantize(uint8_t loopIndex, uint8_t gridDivision) {
  ...
  _sortEvents(loop);
  DBGF("[Loop] Quantized '%s' to 1/%d grid\n", loop.name, gridDivision);
}
...
void LoopEngine::_sortEvents(Loop& loop) {
  // Simple insertion sort (events are usually nearly sorted)
